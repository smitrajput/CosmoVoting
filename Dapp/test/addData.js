...
async function getAccounts() {
  ...
  ElectionInstance = await new web3.eth.Contract(
    Election.abi,
    "0x47eCB5374B46e6645E5A1BB076219C070B7A69ce"
  );
  ElectionInstance.methods
    .addConstituency("roorkee")
    .send({ from: ec_head })
    .then(receipt => {
      console.log(receipt);
  ...
  await ElectionInstance.methods
    .registerVote(uuid_hash_1, "roorkee", "congress", vote_hash1)
    .send({ from: voter1, gas: 3000000 });
  await ElectionInstance.methods
    .registerVote(uuid_hash_2, "roorkee", "congress", vote_hash2)
    .send({ from: voter2, gas: 3000000 });
  await ElectionInstance.methods
    .registerVote(uuid_hash_3, "roorkee", "bsp", vote_hash3)
    .send({ from: voter3, gas: 3000000 });
  ...
  var roorkee_bjp = await ElectionInstance.methods.getVoteCount.call(
    "roorkee",
    "bjp",
    { from: voter1 }
  );
  var roorkee_bsp = await ElectionInstance.methods.getVoteCount.call(
    "roorkee",
    "bsp",
    { from: voter1 }
  );
  var roorkee_congress = await ElectionInstance.methods.getVoteCount.call(
    "roorkee",
    "congress",
    { from: voter1 }
  );

  var winner_roorkee = await ElectionInstance.methods.getWinner.call(
    "roorkee",
    { from: ec_head }
  );

  var roorkee_congress_votes = await ElectionInstance.methods.getVoteHashes.call(
    "roorkee",
    "congress",
    { from: voter1 }
  );
  var roorkee_bsp_votes = await ElectionInstance.methods.getVoteHashes.call(
    "roorkee",
    "bsp",
    { from: voter1 }
  );
  var roorkee_bjp_votes = await ElectionInstance.methods.getVoteHashes.call(
    "roorkee",
    "bjp",
    { from: voter1 }
  );
